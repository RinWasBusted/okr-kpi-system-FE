      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="text-secondary">Bắt đầu: <span className="font-medium text-text">{formatValue(keyResult.start_value || 0)}</span></span>
            <ChevronRight size={12} className="text-secondary" />
            <span className="text-secondary">Hiện tại: <span className={`font-medium ${progress >= 50 ? 'text-cyan-600' : 'text-orange-600'}`}>{formatValue(keyResult.current_value || 0)}</span></span>
            <ChevronRight size={12} className="text-secondary" />
            <span className="text-secondary">Mục tiêu: <span className="font-medium text-emerald-600">{formatValue(keyResult.target_value || 100)}%</span></span>
          </div>
        </div>
